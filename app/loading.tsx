  ...
  return (
    <>
      ...
      <section className="border-b border-game-border-faint bg-[radial-gradient(circle_at_75%_15%,var(--game-hero-glow)_0%,transparent_30%),linear-gradient(135deg,var(--game-hero-start)_0%,var(--game-background)_60%)]">
        <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 sm:px-10 lg:grid-cols-[1fr_0.9fr] lg:px-12 lg:py-24">
          <div className="flex flex-col justify-center">
            <div className="mb-5 h-3 w-56 animate-pulse rounded-sm bg-game-border-faint" />
            <div className="h-16 w-full max-w-xl animate-pulse rounded-sm bg-game-surface-muted sm:h-20" />
            <div className="mt-9 h-11 max-w-xl animate-pulse rounded-sm bg-game-surface-muted" />
          </div>

          <div className="min-h-[360px] animate-pulse border border-game-border-faint bg-game-surface-muted lg:min-h-[470px]" />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-14 sm:px-10 lg:px-12">
        ...
        <div className="mt-3 h-9 w-40 animate-pulse rounded-sm bg-game-surface-muted" />

        <div className="mt-8 flex gap-2 overflow-x-auto pb-2">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-8 w-24 shrink-0 animate-pulse border border-game-border-faint bg-game-surface" />
          ))}
        </div>

        <div className="mt-3 flex flex-wrap gap-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-8 w-16 shrink-0 animate-pulse border border-game-border-faint bg-game-surface" />
          ))}
        </div>
